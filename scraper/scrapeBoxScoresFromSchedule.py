...
# row = df.iloc[10]
# print(row)
# box_score = get_box_scores(row['DATE'], row['HOME_ACRONYM'],  row['VISITOR_ACRONYM'], period='GAME', stat_type='BASIC')
# print(box_score)
...
for index, row in df.iterrows():
    ...
    # Retry mechanism with backoff strategy, from my open ai friend
    retries = 3
    ...
    for attempt in range(retries):
        try:
            ...
            print(
                f"Attempt {attempt+1} failed for {visitor} vs {home} on {date}. Retrying in {wait_times[attempt]} seconds..."
            )
            time.sleep(wait_times[attempt])  # Wait before retrying
    else:
        print(
            f"Failed to retrieve box score for {visitor} vs {home} on {date} after {retries} attempts."
        )
